...
router.get("/bystt/:id", (req, res) => {
    connection.query(
        "SELECT * FROM penjualan where stt=?", req.params.id,
        function (err, rows) {
                ...
                connection.query(
                    ...
                    function (err, rows) {
                        if (!err) {
                            ...
                            return res.status(400).json({ message: "Maaf terjadi kesalahan !, coba ulangi lagi", error: err });
                        }
                    }
                );
            }else
...
router.post("/", middleware.authenticateToken, (req, res) => {
    ...
    connection.query("insert into tracking SET ?",
        data, (err, rows) => {
            ...
            res.status(201).json({ success: true, id:rows.insertId, message: 'Berhasil insert data!' });
        }
    );
});
